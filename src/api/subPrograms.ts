...
export default {
  getELearningDraftSubPrograms: async (): Promise<SubProgramType[]> => {
    ...
    const response: AxiosResponse<any> = await axiosLogged.get(`${baseURL}/subprograms/draft-e-learning`);
    return response.data.data.subPrograms;
  },
  getSubProgram: async (courseId): Promise<SubProgramType> => {
    const baseURL = await Environment.getBaseUrl();
    const response: AxiosResponse<any> = await axiosLogged.get(`${baseURL}/subprograms/${courseId}`);
