...
const CardScreen = ({ exitConfirmationModal, cards, setExitConfirmationModal, resetCardReducer }: CardScreenProps) => {
  ...
  const { stopTimer, navigateNext, setIsActive } = useContext(CardContext);

  const onSwipe = Gesture.Pan().onEnd((event) => {
    if (event.translationX > SWIPE_SENSIBILITY && index > 0 && isLeftSwipeEnabled) {
      runOnJS(router.navigate)(`Courses/ActivityCardContainer/${index - 1}`);
    }
  ...
  if (cards.length === index) return <Redirect href={'/Courses/ActivityCardContainer/cardTemplates/ActivityEndCard'} />;
